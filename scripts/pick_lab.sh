...
SSH() { ssh "${SSH_OPTS[@]}" "$@"; }
FILTER='median|lean\]|reach\]|state\] side|startup|accepted pitch|retry\]|place\]|space\]|grip\]|evidence|torque\]|cleanup|fatal|staged|rest\]|gripper\]|motion\] stage|complete\]'

host() {
  if [ -s "$HOST_CACHE" ]; then
    local cached; cached=$(cat "$HOST_CACHE")
    if SSH "$cached" true 2>/dev/null; then echo "$cached"; return 0; fi
  fi
...
sync_files() {
  ...
  scp -q "${SSH_OPTS[@]}" "${changed[@]}" "$1:$REMOTE/"
}

# Run a motion command detached, so an SSH drop cannot kill it mid-move, then
# follow its log until the runner exits.
# Launch detached (an SSH drop cannot kill a motion), then stream the log over
# the one live connection instead of polling it twice a second.
run_detached() {
  local h="$1"; shift
  SSH "$h" "cd $REMOTE && rm -f pick.log pick.pid && setsid nohup $PY pick_object.py $* \
      --pid-file $REMOTE/pick.pid > $REMOTE/pick.log 2>&1 < /dev/null & sleep 0.5; echo started"
  SSH "$h" "cd $REMOTE; tail -n +1 -f pick.log & TAIL=\$!; \
      for _ in \$(seq 1 400); do sleep 0.5; [ -f pick.pid ] || break; done; \
      sleep 1; kill \$TAIL 2>/dev/null" | grep --line-buffered -E "$FILTER" | cut -c1-170
}
...
\"" ;;
  ...
  plan)  ssh "$H" "cd $REMOTE && $PY -u pick_object.py $* 2>&1 | grep -E '$FILTER'" | cut -c1-170 ;;
  pick)  run_detached "$H" --execute --adjust --auto-space "$@" ;;
