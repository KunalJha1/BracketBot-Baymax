...
# --- demo and follow arbitration -------------------------------------------


def test_follow_refuses_to_start_while_the_judge_demo_runs():
    controller = RobotController(("not-used",), simulate=True)
    controller.state.demo_active = True

    ok, message = controller.set_follow(True)

    assert ok is False
    assert "judge demo" in message
    assert follow_off(controller)


def test_judge_demo_refuses_to_start_while_following():
    controller = RobotController(("not-used",), simulate=True)
    controller.state.follow_enabled = True

    ok, message = controller.start_demo()

    assert ok is False
    assert "Stop following" in message
    assert controller.state.snapshot()["demo"]["active"] is False
